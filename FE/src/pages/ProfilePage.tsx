...
export default function ProfilePage() {
  ...
  }, [user?.id, user?.full_name, user?.fullName]);
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await authService.updateMe({ full_name: fullName.trim() || null });
      await refreshUser();
              ...
              <div className="flex items-end gap-6 -mt-12">
                <div className="relative">
                  <Avatar className="h-24 w-24 border-4 border-card">
                    <AvatarImage src={user?.avatarUrl ?? user?.avatar_url ?? undefined} alt={displayName} />
                    <AvatarFallback className="text-2xl bg-primary/10 text-primary">{displayName?.charAt(0)}</AvatarFallback>
                  </Avatar>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className="text-xs text-muted-foreground">
                      ...
                      {user?.createdAt ?? user?.created_at
                        ? parseBackendDate(user.createdAt ?? user.created_at!).toLocaleDateString(language === 'vi' ? 'vi-VN' : 'en-US')
                        : '—'}
                    </span>
                  </div>
                </div>
