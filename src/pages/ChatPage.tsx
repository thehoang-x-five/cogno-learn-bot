...
export default function ChatPage() {
  ...
  const handleDeleteConversation = () => {
    ...
    toast({ title: 'Đã xóa', description: `Cuộc trò chuyện "${deleteConvTarget.title}" đã được xóa.` });
  ...
  const handleNewConversation = () => {
    const newConv: Conversation = {
      ...
      title: 'Cuộc trò chuyện mới', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), lastMessage: '',
    };
    setConversations((prev) => [newConv, ...prev]);
    setActiveConversation(newConv.id);
    setMessages([]);
    toast({ title: 'Đã tạo', description: 'Cuộc trò chuyện mới đã được tạo.' });
  };
  ...
  return (
    <div className="flex h-screen">
        ...
        <div className="border-t p-4 bg-card/80 backdrop-blur-sm">
          <div className="max-w-3xl mx-auto">
            <div className="flex items-center gap-2 p-2 rounded-xl bg-secondary/50 border focus-within:ring-2 focus-within:ring-primary/30 focus-within:border-primary/30 transition-all">
              <Button variant="ghost" size="icon" className="shrink-0 h-8 w-8" onClick={() => toast({ title: 'Đính kèm file', description: 'Kéo thả file vào ô chat hoặc sử dụng trang Tài liệu để upload.' })}>
                <Paperclip className="h-4 w-4 text-muted-foreground" />
              </Button>
              <Input ref={inputRef} value={inputValue} onChange={(e) => setInputValue(e.target.value)} onKeyDown={handleKeyDown}
                placeholder="Nhập câu hỏi của bạn..." className="border-0 bg-transparent focus-visible:ring-0 flex-1 h-8 text-sm" />
      ...
      <ConfirmDeleteDialog
        ...
        description={`Bạn có chắc chắn muốn xóa "${deleteConvTarget?.title}"? Tất cả tin nhắn trong cuộc trò chuyện sẽ bị mất.`}
